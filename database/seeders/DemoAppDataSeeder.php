...
class DemoAppDataSeeder extends Seeder
{
    ...
    {
        $models = [
            'Subject', 'Student', 'Registration', 'StudentAttendance', 'EmployeeAttendance',
            'Exam', 'Grade', 'ExamRule', 'Mark', 'Result', 'Leave', 'UserRole', 'Employee', 'User',
            'AppMeta', 'AcademicYear', 'IClass', 'Section'
        ];
        foreach ($models as $model) {
            class_alias('App\\Models\\' . $model, 'App\\' . $model);
            class_alias('App\\Models\\' . $model, 'Database\\Seeders\\App\\' . $model);
        }
    ...
    private function teacherData() {
        $created_by = 1;
        $tz = env('APP_TIMEZONE','Asia/Dhaka');
        ...
        $created_at_old = Carbon::now($tz)->subDays(60);
        $created_at_new = Carbon::now($tz)->subDays(10);

        // Head (old)
        $instituteHead = factory(App\Employee::class)->create([
            'designation' => $insHeadDesignation,
            'created_by' => $created_by,
            'created_at' => $created_at_old
        ]);
        \App\UserRole::create([
            'user_id' => $instituteHead->user_id,
            'role_id' => AppHelper::USER_TEACHER,
            'created_by' => $created_by,
            'created_at' => $created_at_old
        ]);

        // 5 Teachers (4 old, 1 new)
        for ($i = 1; $i <= 5; $i++) {
            $created_at = ($i <= 4) ? $created_at_old : $created_at_new;
    ...
    private function employeeData() {
        ...
        // 9 employees (6 old, 3 new)
        $roles = [5, 6, 7];
        for ($i = 1; $i <= 9; $i++) {
            $created_at = ($i <= 6) ? $created_at_old : $created_at_new;
            $role_id = $roles[($i - 1) % count($roles)];
    ...
    private function subjectData()
    {
        ...
        $getCreatedAt = function() use (&$subjectCount, $created_at_old, $created_at_new) {
            $subjectCount++;
            return ($subjectCount <= 20) ? $created_at_old : $created_at_new;
        };

        $subject = factory(App\Subject::class, 10)->make();
        foreach ($subject as $sub) {
            $sub->created_by = $created_by;
            $sub->created_at = $getCreatedAt();
            $sub->save();
        ...
        // Bangla 1st
        $sub = factory(App\Subject::class)->make(['class_id' => 1, 'type' => 1, 'name' => 'Bangla 1st', 'code' => '101']);
        $sub->created_by = $created_by; $sub->created_at = $getCreatedAt(); $sub->save();
        $sub->teachers()->attach(App\Employee::where('role_id', \App\Http\Helpers\AppHelper::USER_TEACHER)->inRandomOrder()->first()->id);

        // Bangla 2nd
        $sub = factory(App\Subject::class)->make(['class_id' => 1, 'type' => 1, 'name' => 'Bangla 2nd', 'code' => '102']);
        $sub->created_by = $created_by; $sub->created_at = $getCreatedAt(); $sub->save();
        $sub->teachers()->attach(App\Employee::where('role_id', \App\Http\Helpers\AppHelper::USER_TEACHER)->inRandomOrder()->first()->id);

        // English 1st
        $sub = factory(App\Subject::class)->make(['class_id' => 1, 'type' => 1, 'name' => 'English 1st', 'code' => '107']);
        $sub->created_by = $created_by; $sub->created_at = $getCreatedAt(); $sub->save();
        $sub->teachers()->attach(App\Employee::where('role_id', \App\Http\Helpers\AppHelper::USER_TEACHER)->inRandomOrder()->first()->id);

        // English 2nd
        $sub = factory(App\Subject::class)->make(['class_id' => 1, 'type' => 1, 'name' => 'English 2nd', 'code' => '108']);
        $sub->created_by = $created_by; $sub->created_at = $getCreatedAt(); $sub->save();
        $sub->teachers()->attach(App\Employee::where('role_id', \App\Http\Helpers\AppHelper::USER_TEACHER)->inRandomOrder()->first()->id);

        // Math
        $sub = factory(App\Subject::class)->make(['class_id' => 1, 'type' => 1, 'name' => 'Math', 'code' => '111']);
        $sub->created_by = $created_by; $sub->created_at = $getCreatedAt(); $sub->save();
        $sub->teachers()->attach(App\Employee::where('role_id', \App\Http\Helpers\AppHelper::USER_TEACHER)->inRandomOrder()->first()->id);

        // Computer
        $sub = factory(App\Subject::class)->make(['class_id' => 1, 'type' => 1, 'name' => 'Computer', 'code' => '112']);
        $sub->created_by = $created_by; $sub->created_at = $getCreatedAt(); $sub->save();
        $sub->teachers()->attach(App\Employee::where('role_id', \App\Http\Helpers\AppHelper::USER_TEACHER)->inRandomOrder()->first()->id);

        echo PHP_EOL."seeding class two subjects";
        // Bangla 1st class 2
        $sub = factory(App\Subject::class)->make(['class_id' => 2, 'type' => 1, 'name' => 'Bangla 1st', 'code' => '101']);
        $sub->created_by = $created_by; $sub->created_at = $getCreatedAt(); $sub->save();
        $sub->teachers()->attach(App\Employee::where('role_id', \App\Http\Helpers\AppHelper::USER_TEACHER)->inRandomOrder()->first()->id);

        // Magic Study
        $sub = factory(App\Subject::class)->make(['class_id' => 2, 'type' => 3, 'name' => 'Magic Study', 'code' => '102']);
        $sub->created_by = $created_by; $sub->created_at = $getCreatedAt(); $sub->save();
        $sub->teachers()->attach(App\Employee::where('role_id', \App\Http\Helpers\AppHelper::USER_TEACHER)->inRandomOrder()->first()->id);

        // English 1st class 2
        $sub = factory(App\Subject::class)->make(['class_id' => 2, 'type' => 1, 'name' => 'English 1st', 'code' => '107']);
        $sub->created_by = $created_by; $sub->created_at = $getCreatedAt(); $sub->save();
        $sub->teachers()->attach(App\Employee::where('role_id', \App\Http\Helpers\AppHelper::USER_TEACHER)->inRandomOrder()->first()->id);

        // Drawing
        $sub = factory(App\Subject::class)->make(['class_id' => 2, 'type' => 3, 'name' => 'Drawing', 'code' => '108']);
        $sub->created_by = $created_by; $sub->created_at = $getCreatedAt(); $sub->save();
        $sub->teachers()->attach(App\Employee::where('role_id', \App\Http\Helpers\AppHelper::USER_TEACHER)->inRandomOrder()->first()->id);

        // Math class 2
        $sub = factory(App\Subject::class)->make(['class_id' => 2, 'type' => 1, 'name' => 'Math', 'code' => '111']);
        $sub->created_by = $created_by; $sub->created_at = $getCreatedAt(); $sub->save();
        $sub->teachers()->attach(App\Employee::where('role_id', \App\Http\Helpers\AppHelper::USER_TEACHER)->inRandomOrder()->first()->id);

        // Computer class 2
        $sub = factory(App\Subject::class)->make(['class_id' => 2, 'type' => 2, 'name' => 'Computer', 'code' => '112']);
        $sub->created_by = $created_by; $sub->created_at = $getCreatedAt(); $sub->save();
        $sub->teachers()->attach(App\Employee::where('role_id', \App\Http\Helpers\AppHelper::USER_TEACHER)->inRandomOrder()->first()->id);
    }
    ...
    private function studentData() {
        ...
        $students = collect();
        for ($i = 1; $i <= 15; $i++) {
            $created_at = ($i <= 13) ? Carbon::now($tz)->subDays(45) : Carbon::now($tz)->subDays(5);
            $student = factory(App\Student::class)->create(['created_by' => $created_by, 'created_at' => $created_at]);
